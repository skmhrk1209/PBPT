...
template <
    typename Geometry, typename Scalar = double, template <typename, auto, auto> typename Matrix = pbpt::tensor::Matrix>
struct Rotation {
  constexpr Rotation() = default;
  constexpr Rotation(const Geometry &geometry, const Matrix<Scalar, 3, 3> &rotation)
      : m_geometry(geometry), m_rotation(rotation) {}
  constexpr Rotation(Geometry &&geometry, Matrix<Scalar, 3, 3> &&rotation)
      : m_geometry(std::move(geometry)), m_rotation(std::move(rotation)) {}
  ...
  constexpr auto intersect(const auto &ray) const {
    ...
    auto rotate_normal = [&]<typename Intersection>(const Intersection &intersection) constexpr -> Intersection {
      auto rotated_normal_evaluator =
          [this, normal_evaluator = intersection.surface().normal_evaluator()](const auto &position) constexpr {
...
constexpr auto make_rotation(Geometry &&geometry, auto &&...args) {
  return Rotation<std::decay_t<Geometry>, Scalar, Matrix>(
      std::forward<Geometry>(geometry), std::forward<decltype(args)>(args)...
  );
}
...
constexpr auto make_rotation_matrix(const auto &axis, auto angle) {
  ...
  return (1.0 - cos) *
             Matrix<Scalar, 3, 3>{
                 Vector<Scalar, 3>{x * x, x * y, x * z}, Vector<Scalar, 3>{y * x, y * y, y * z},
                 Vector<Scalar, 3>{z * x, z * y, z * z}
             } +
         Matrix<Scalar, 3, 3>{
             Vector<Scalar, 3>{cos, -z * sin, y * sin}, Vector<Scalar, 3>{z * sin, cos, -x * sin},
