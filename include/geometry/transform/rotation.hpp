...
constexpr auto make_rotation_matrix(const auto &axis, auto angle) {
  ...
  return (1.0 - cos) * Matrix<Scalar, 3, 3>{Vector<Scalar, 3>{x * x, x * y, x * z}, Vector<Scalar, 3>{y * x, y * y, y * z},
                                            Vector<Scalar, 3>{z * x, z * y, z * z}} +
         Matrix<Scalar, 3, 3>{Vector<Scalar, 3>{cos, -z * sin, y * sin}, Vector<Scalar, 3>{z * sin, cos, -x * sin},
