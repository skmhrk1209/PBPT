    ...
    constexpr auto intersect(const auto &ray) const {
        ...
        auto cylinder_normal = [this](const auto &position) constexpr -> Vector<Scalar, 3> {
            ...
            auto [normal_z, normal_x]
                = coex::tensor::normalized(Vector<Scalar, 2>{position_z, position_x} / coex::tensor::elemwise(coex::math::square<Scalar>, m_radii));
            ...
            if (-m_height <= min_intersection_y && min_intersection_y <= m_height) {
                if (-m_height <= max_intersection_y && max_intersection_y <= m_height) {
                    ...
                    if (max_distance > 0.0) {
                        ...
                        if (coex::tensor::dot(norm_max_intersection, norm_max_intersection) <= 1.0) {
                ...
                if (max_distance > 0.0) {
                    if (auto intersection = circle_position(ray_direction_y > 0 ? -m_height : m_height)) {
                        ...
                        if (coex::tensor::dot(norm_min_intersection, norm_min_intersection) <= 1.0) {
