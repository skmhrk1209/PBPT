...
inline constexpr auto object = []() constexpr {
    ...
    return pbpt::geometry::csg::make_union(
        ...
        pbpt::geometry::transform::make_translation(
            pbpt::geometry::primitive::make_ellipsoid<>(pbpt::tensor::Vector<Scalar, 3>{1000.0, 1000.0, 1000.0}, pbpt::material::make_lambertian<>(pbpt::tensor::Vector<Scalar, 3>{0.5, 0.5, 0.5})),
            pbpt::tensor::Vector<Scalar, 3>{0.0, 1000.0, 0.0}),
        pbpt::geometry::csg::make_union(
            // left sphere (gold)
            pbpt::geometry::transform::make_translation(
                pbpt::geometry::primitive::make_ellipsoid<>(pbpt::tensor::Vector<Scalar, 3>{1.0, 1.0, 1.0}, pbpt::material::make_metal<>(pbpt::tensor::Vector<std::complex<Scalar>, 3>{
                                                       0.18299 + 3.42420i,
                                                       0.42108 + 2.34590i,
                                                       1.37340 + 1.77040i,
                                                   })),
            ...
            pbpt::geometry::csg::make_union(
                // center sphere (glass)
                pbpt::geometry::transform::make_translation(pbpt::geometry::primitive::make_ellipsoid<>(pbpt::tensor::Vector<Scalar, 3>{1.0, 1.0, 1.0}, pbpt::material::make_dielectric<>(1.5)),
                    ...
                    pbpt::geometry::csg::make_union(
                        ...
                        [function =
                             [&]<auto I, auto... Is>(auto self, std::index_sequence<I, Is...>) constexpr {
                                 auto [coord_x, coord_z] = pbpt::random::uniform_in_unit_circle<>(generator) * 10.0;
                                 auto position = pbpt::tensor::Vector<Scalar, 3>{coord_x, -0.2, coord_z};
                                 auto reflectance = pbpt::tensor::elemwise(
                                     pbpt::math::square<Scalar>, pbpt::tensor::Vector<Scalar, 3>{pbpt::random::uniform(generator, 0.0, 1.0),
                                                                                                 pbpt::random::uniform(generator, 0.0, 1.0),
                                                                                                 pbpt::random::uniform(generator, 0.0, 1.0)});
                                 auto sphere = pbpt::geometry::transform::make_translation(
                                     pbpt::geometry::primitive::make_ellipsoid<>(pbpt::tensor::Vector<Scalar, 3>{0.2, 0.2, 0.2}, pbpt::material::make_lambertian<>(std::move(reflectance))),
                                     std::move(position));

                                 if constexpr (sizeof...(Is))
                                     return pbpt::geometry::csg::make_union(std::move(sphere), self(self, std::index_sequence<Is...>{}));
                                 else
                                     return sphere;
                        ...
                        pbpt::geometry::csg::make_union(
                            ...
                            [function =
                                 [&]<auto I, auto... Is>(auto self, std::index_sequence<I, Is...>) constexpr {
                                     ...
                                     auto sphere = pbpt::geometry::transform::make_translation(
                                         pbpt::geometry::primitive::make_ellipsoid<>(pbpt::tensor::Vector<Scalar, 3>{0.2, 0.2, 0.2}, pbpt::material::make_dielectric<>(refractive_index)),
                                         std::move(position));
                            ...
                            [function = [&]<auto I, auto... Is>(auto self, std::index_sequence<I, Is...>) constexpr {
                                ...
                                pbpt::tensor::Vector<std::complex<Scalar>, 3> refractive_index{
                                    pbpt::random::uniform(generator, 0.0, 5.0) + pbpt::random::uniform(generator, 0.0, 5.0) * 1i,
                                    pbpt::random::uniform(generator, 0.0, 5.0) + pbpt::random::uniform(generator, 0.0, 5.0) * 1i,
                                    pbpt::random::uniform(generator, 0.0, 5.0) + pbpt::random::uniform(generator, 0.0, 5.0) * 1i,
                                };
                                auto sphere = pbpt::geometry::transform::make_translation(
                                    pbpt::geometry::primitive::make_ellipsoid<>(pbpt::tensor::Vector<Scalar, 3>{0.2, 0.2, 0.2}, pbpt::material::make_metal<>(std::move(refractive_index))),
                                    std::move(position));
...
inline constexpr auto background = [](const auto &ray) constexpr {
    return pbpt::math::lerp(pbpt::tensor::get<1>(ray.direction()), -1.0, 1.0, pbpt::tensor::Vector<Scalar, 3>{0.5, 0.75, 1.0},
